...
- **ARCH-A7: `/sdd batch` qa-gate exit silently loses ESCALATE**
  ...
  - Source: `design/03-flow-design.md` §7.3 vs `design/stage-designs/test.md` §6, §8, §9.
  - Fix: Either set 5-keys including `qa` for batch (deviating from spec/flow/batch.md §5's 4-key contract — breaks PRESERVE), OR define batch-specific main-session behavior: on `OK NEEDS_MANUAL_QA` / `ESCALATE` inside a `claude -p` child, exit child cleanly with `OK PAUSE`-equivalent status. Document explicitly.
