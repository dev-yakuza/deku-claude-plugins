...
- `model`: `sonnet` when `depth = shallow`, otherwise `opus` (both `deep` and `default` resolve to `opus`). The whole stage (analysis work + inlined reviewers) runs in this one sub-agent; analyze has no in-context `/security-review` / `/code-review`, so `opus` at the deep dial handles What/Why + ambiguity reasoning, and `sonnet` at the shallow dial cuts cost on explicitly low-stakes Issues (per `_review_helpers.md` Section A.2.1).
...
3. Branch on user choice:
   - **Continue** → re-spawn `stage_analyze` with `Resume: continue-after-escalation`:
     - `subagent_type`: `general-purpose`, `model`: `sonnet` when `depth = shallow`, else `opus` (same rule as Phase 1), `description`: `stage_analyze resume for #$1`
     - `prompt`:
       > Read `<<SKILL_DIR>>/commands/atoms/stage_analyze.md` and execute its instructions for Issue #$1.
       >
       > Inputs:
...
- **Reviewers run serially inside the sub-agent.** Reviewer independence is preserved by the sub-agent's internal narrative structure (work output is shared ground truth — no re-fetch; no cross-visibility of verdicts).
- **Label transitions are main session's responsibility.** `stage_analyze` never sets labels itself.
- **Depth label override**: `sdd:review:deep` / `sdd:review:shallow` selects the depth dial, which sets this stage's spawn model — `deep → opus`, `shallow → sonnet`, `default → opus` per `_review_helpers.md` Section A.2.1. The dial also informs the sub-agent's reasoning style per Section A.2.
