...
- **Reviewers run serially inside the sub-agent.** Reviewer independence is preserved by the sub-agent's internal narrative structure (work output is shared ground truth — no re-fetch for SDD reviewers; `parent_integration_review` always fetches cross-Issue; no cross-visibility of verdicts). PARENT path runs 4 reviewers (3 SDD + `parent_integration_review`); SINGLE/CHILD runs 3.
