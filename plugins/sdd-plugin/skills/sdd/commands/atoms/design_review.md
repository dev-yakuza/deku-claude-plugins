...
8. **Post a review comment** with marker `<!-- sdd:review:design:<role> -->`. Standard duplicate-prevention.

   Comment body follows the standard format (see `analyze_review.md` template) with stage=design and role=`<role>`. Include the `<!-- sdd:findings:json -->` block per `_review_helpers.md` Section B.
