...
echo ""
echo "== H. 훅 매니저 소유 저장소 (lefthook/husky) =="
# 실제 저장소(word_app, one-man-company)가 이 형태다. 매니저가 .git/hooks/pre-commit 을
# 생성·소유하므로 Guild 가 그 파일을 가로채면 다음 `lefthook install` 에 조용히 지워진다.
# 탐지가 정확해야 init 이 "가로채기" 대신 "매니저 설정에 등록" 경로를 고른다.
detect() { printf '%s' "$1" | python3 "$GATE" --detect-hook-manager 2>/dev/null; }
...
for pair in "lefthook:$LEFTHOOK_HOOK" "husky:$HUSKY_HOOK" "pre-commit:$PRECOMMIT_HOOK"; do
  want="${pair%%:*}"; body="${pair#*:}"
  got="$(detect "$body")"
  [ "$got" = "$want" ] && ok "$want 훅을 매니저로 판별" || bad "$want 훅을 매니저로 판별" "$want" "$got"
done
got="$(detect "$HANDWRITTEN")"
[ "$got" = "none" ] && ok "손으로 쓴 훅은 none (체이닝 경로)" || bad "손으로 쓴 훅은 none" "none" "$got"
got="$(detect "")"
[ "$got" = "none" ] && ok "훅 없음은 none" || bad "훅 없음은 none" "none" "$got"
got="$(detect "$(cat "$HOOK")")"
[ "$got" = "guild" ] && ok "Guild 자신의 shim 은 guild (제자리 덮어쓰기)" || bad "Guild shim 판별" "guild" "$got"

# 매니저 경로에서는 게이트가 매니저 명령으로 등록돼 동작해야 한다.
new_repo h
mkdir -p .claude/guild/gates/scripts .claude/guild/memory
printf '{"gates":{"enabled":true}}' > .claude/guild/config.json
cp "$GATE" .claude/guild/gates/scripts/gate_precommit.py
HPH="$(git rev-parse --git-path hooks/pre-commit)"
# 매니저가 소유한 훅을 흉내: 자기 훅에서 Guild 게이트를 한 명령으로 호출
printf '#!/bin/sh\n# lefthook (simulated)\npython3 .claude/guild/gates/scripts/gate_precommit.py --git-hook || exit 1\nexit 0\n' > "$HPH"
chmod +x "$HPH"
printf 'SECRET=abc\n' > .env; git add .env >/dev/null 2>&1
if git commit -qm secret >/dev/null 2>&1; then
  bad "매니저 명령으로 등록된 게이트가 차단" "block" "allow"
else
  ok "매니저 명령으로 등록된 게이트가 차단"
fi
...
INST_MIN_CHECKS=28
if [ "$((PASS + FAIL))" -lt "$INST_MIN_CHECKS" ]; then
  echo "FAIL  실행된 검사가 $((PASS + FAIL))건뿐입니다 (최소 ${INST_MIN_CHECKS}건)."
  exit 1
fi
[ "$FAIL" -eq 0 ]
