...
- **활성 작업** — open Issues bucketed by **stage** (analyze/design/execute/test/qa + children, i.e. `guild:children` split parents in their orchestration state — mutually exclusive, one per Issue) + `guild:done` recent. Derive each Issue's stage with the **list form of `_handoff.md` Section A — canonical stage derivation**, which yields one `stage` label (or `"none"`) plus the additive `paused` flag, having excluded `guild:child` (the permanent identity marker every child carries *alongside* its real stage label), `guild:needs-human` and `guild:harness` — none of which is a stage:
  ```bash
  gh issue list --state open --limit 200 --json number,title,labels --jq '[.[] | {number, stage: ([.labels[].name] | map(select(startswith("guild:") and . != "guild:child" and . != "guild:needs-human" and . != "guild:harness")) | .[0] // "none"), paused: ([.labels[].name] | any(. == "guild:needs-human"))}]'
  ```
  Count the buckets by `stage` value (`"none"` = an open Issue Guild hasn't entered yet — includes a filed-but-unstarted `guild:harness` remediation Issue). **`guild:needs-human` is a cross-cutting count, not a same-level bucket**: it's additive, so summing it beside the stage buckets double-counts a paused Issue. Render the `paused: true` count as its own highlighted line ("N개 이슈 사람 대기 중"), consistent with this file's Hard rule below ("surface ... `guild:needs-human` Issues ... before the static counts").
