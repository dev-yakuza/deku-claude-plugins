...
> **Output language**: all human-readable output in `config.language` (`_handoff.md` Section K); **every** sub-agent prompt below ends with "Write output in `config.language`" — never drop it from a spawn. ⚠ One documented exception sits inside Section K, not here: the **unattended narration carve-out** (`dev.md` drive path only) lets free narration before the `>>> RESULT <<<` sentinel be ASCII English. It is attached by the leader as a conditional extra line; the closing language sentence above stays regardless.
...
⚠ **Never discount this Issue's OWN work under that path — committed or not.** The discount exists because Guild's *other* steps (and other Issues) strand files there, not because the path is uninteresting. An Issue whose deliverable genuinely is a spec change may have that work committed, or sitting uncommitted because a hook blocked the commit — the spine says elsewhere that this is routine, so committed-ness is the wrong test. Judge by whose work it is: this Issue's developer produced it → audit it; design/another Issue produced it → discount it. Getting this backwards makes a spec-deliverable Issue fail with "nothing to audit" on work that was actually done. ⚠ **Cannot tell → audit it, and say so.** You usually can tell — you hold the developer's `>>> RESULT <<<` and know what this stage wrote — so this is the residual case, not the default. But the two errors are not the same size, and this file has already chosen between them once: a wrongly-audited path costs a finding about something the developer did not write (noisy, visible, dismissible attended; unattended a pause with the evidence attached — *"a paused Issue over an unaudited change that ships"*), while a wrongly-discounted one is invisible by construction, and can end as a `FAIL: nothing to audit` on an Issue whose work was real. So include it, set the spec-deliverable line to the reviewing form, and note in the leader's own record — and in the escalation, if one follows — that the provenance was unresolved, so the reader can weigh the finding accordingly instead of reading it as settled.
...
- `prompt`:
  > Adopt the persona in `.claude/agents/<role>.md`. Review the implementation on the current branch for Issue #<N> from your specialty. You are reviewing the DEVELOPER's diff, not your own work (external, adversarial). Read the part of `docs/specs/<N>/` that bears on **your specialty** for design/intent — `ls` it and take those sections, not the directory. You review one slice, not the whole intent, so a whole-directory load here is re-billed on every turn that follows.
  > <!-- guild:result-contract -->
  > Return EXACTLY one status line, preceded by a `>>> RESULT <<<` sentinel on its own line. Anything before the sentinel is ignored. Status is one of `DONE` / `DONE_WITH_CONCERNS: <one-line>` / `BLOCKED: <one-line>` / `NEEDS_CONTEXT: <one-line>` / `FAIL: <reason>`. **Artifacts are passed as files, not pasted** — write to the working tree or `docs/specs/<issue>/` and name the path in the RESULT line; never inline an artifact body into it.
  > <!-- /guild:result-contract -->
...
⚠ **Run the push in the FOREGROUND with an explicit `timeout`, never `run_in_background`** (`_bash_rules.md` — *Long-running commands*). A repo whose pre-push hook mirrors CI takes minutes, which outruns the Bash tool's 120 s default — and a child that backgrounds it ends its turn waiting for a completion notification that a `claude -p` process can never deliver, because it kills its own background tasks when the response completes. The push then never lands, so there is no PR, the stage cannot advance, and the supervisor re-resumes into the same dead end. Measured: this cost a sprint member outright and left two more branches unpublished.
...
- **Every spawn is released once consumed** (`_handoff.md` Section C, "Releasing the sub-agent"): developer, tech-lead, 3.5b roles → `TaskStop` after their RESULT is read; the 3.5a auditor → `TaskStop` after the post-scan mutation readings are taken. `ListAgents` at Step 6 shows none of this stage's agents.
