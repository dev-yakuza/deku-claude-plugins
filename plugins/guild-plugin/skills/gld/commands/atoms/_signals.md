...
**Design rule — hybrid capture (the ① decision · kill-gate PASS):**
...
Every signal that will drive a change MUST be anchored to **real ground truth**: an objective outcome (test / gate / CI result) or a **real human action** (correction, revert, PR reject). (back-patting 방지.)
...
- **Cross-role reversal ≠ self-review (the agent↔agent capture — insider/outsider).** A *different* role overturning another role's confident output — a **design-stage specialist `BLOCKED`** reversing a decided approach (designer WCAG, dba integrity, security threat), tech-lead/gate `BLOCKED` on the developer's diff, or QA/designer finding a defect the test stage passed — **is** legitimate ground truth **when anchored to an objective outcome** (a measured ratio, a concrete non-conformance, a real security/a11y defect, or raw runner output contradicting a claimed green). That anchor is what separates it from self-review; external cross-role review is exactly what the insider/outsider stance relies on. Weight it **below** a real human correction, above an unanchored opinion. This is where the "몸통" of the correction distribution lives (the human-override capture only sees the "꼬리").
- **"귀찮아서 기각" ≠ "틀려서 기각".** A rejection is a strong signal only with a stated reason, or a subsequent real defect in that area (rule-loop anchor). Record the reason when known; absent a reason, weight low.
...
| verify self-report ↔ raw-runner gap | verify gate (`_handoff.md` Section E) | `verify-gap` — claimed vs raw · `surprise` |
...
- **Location**: `.claude/guild/memory/ground-truth.jsonl` — episodic tier (④), **gitignored** (init already gitignores `memory/`). Append-only; one JSON object per line (machine-parseable; human-scannable with `tail`).
- **Commit vs gitignore is still open** (team-share benefit vs leak/noise risk). ① keeps it gitignored (matches init default = lowest risk); revisit when the working-tier read is built.
- **Entry schema** (one line each):
  ...
  ```
  - `area` = the retrieval key for runtime working-memory (`_preflight.md` Item 8). Optional; null when the signal isn't tied to a path (e.g. a scope-interpretation correction).
  - `surprise:true` when the human overturned a choice the agent was confident in, **or** a claimed-pass was actually red (this is the ranking lever the kill-gate validated: A1 "guard existed yet bug escaped", A3 "confident work reversed" ranked top).
...
Frequency discipline (all readers): repeated ≥K only (drop 1-offs), evidence required (no evidence → not a signal), local & read-only.

## Section F — Bundled transcript parser (`scan_transcript.py`)

Fragile source → one bundled command (not atomic-bash pipes). Invoked as a single Bash call:
```bash
python3 <<SKILL_DIR>>/commands/atoms/scan_transcript.py --repo-cwd <abs-repo-path>
```
- **No time-window flag** — frequency = distinct **session count** (≥K), not recency; a `--since-days` filter over the fragile transcript timestamps was error-prone (undated sessions, mixed ISO formats) for no benefit, so staleness is left to the evolve ledger skip-list.
...
- **Best-effort + graceful degrade** — unparseable lines are skipped; if the dir/format cannot be read at all, exit non-zero with a one-line reason → the caller drops transcript signals and proceeds on the durable backbone. **Exit codes** (the caller may branch on these, or simply treat any non-zero as "no transcript signals this run"): `0` parsed · `1` unexpected exception · `2` no `~/.claude/projects` directory · `3` no transcript recorded this repo's cwd.
- **`degraded` is a real value, not decoration.** It is `true` whenever the scan could not see everything it should have — no source directory, no cwd match, an aborted run, or a *partial* parse (some matched transcripts unreadable, which makes every `≥K sessions` threshold an under-count). When `true` the block also carries a `reason` string. A RESULT block is now emitted on **every** path, including the exit-2/exit-3 ones, so a caller reading stdout can distinguish **"this repo genuinely has no friction"** from **"this scan saw nothing"** — previously `degraded` was hardcoded `false` with no code path able to set it, and the two cases were indistinguishable. ⚠ `degraded: true` with an empty `signals` array is **not** evidence of a clean repo and must never be counted as such by the data-sufficiency gate.
...
- **Output**: `>>> RESULT <<<` + JSON:
  ...
  { "feasibility": "<=1 line — parse success/limits", "degraded": false,
    "reason": "<=1 line — present only when degraded is true>",
