...
**Read-only human-correction reader for `/gld evolve` Phase 1.** One of the four evolve scan atoms (`_signals.md` Section E). Corrections — things a **human** overturned — are the **strongest** evolve signal ("사람이 되돌린 것 = 가장 강한 신호"). This atom aggregates them from three sources and **anchors every one to real ground truth** (Section B). Independently spawnable; returns a compact `>>> RESULT <<<` JSON.
