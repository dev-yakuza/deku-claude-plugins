...
- **Attempt 2** (the retry within the bounded ~2-loop cap) runs **one tier above the default** for the roles re-invoked in that retry **that this rule names** — the developer **and** the tech-lead/gate role whose `BLOCKED` triggered the loop-back (execute Step 3.5a's always-on auditor is also re-invoked, but it issues no `BLOCKED`, so it is never the trigger and stays at its default tier), so the re-check isn't weaker than the redo. Sonnet → **Opus**. If a role's default was already Opus, it stays at Opus (no higher tier; this is not a failure).
