...
| `<!-- guild:auditor:execute -->` … `<!-- /guild:auditor:execute -->` | **execute only** (`_execute_spine.md` Step 4) | the always-on external auditor's findings and their disposition — **four** ASCII tokens, `looped-back` (blocking now, outcome pending) / `fixed` / `recorded` (not retried) / `dismissed` (reason required); `_execute_spine.md` Step 4 defines them and the re-statement rule that settles a `looped-back` in the next block. ⚠ **Cumulative** (the Section B exception above): each write fetches the existing `.body`, appends a `### audit-record <n>` block (ASCII token — Section K) numbered by counting the blocks already there, and leaves earlier blocks intact — execute is re-entered from `test`/`qa` as a fresh invocation whose leader remembers nothing, and a replacing PATCH would erase an earlier dismissal. This is the **record of account**; the PR body's `<!-- guild:auditor:pr -->` block is a reviewer-facing copy of it. |
| `<!-- guild:auditor-violation -->` … `<!-- /guild:auditor-violation -->` | **execute only** (`_execute_spine.md` Step 3.5a) | written only when the external auditor mutated the repo in breach of its read-only contract: the before/after `git diff --numstat --no-renames <mb>` and `git status --porcelain -uall` readings, the pre-spawn `git rev-parse HEAD` value, and the changed paths. The branch is deliberately left mutated (INV3 forbids an automated repair), so this is the evidence a human needs to revert it. ⚠ **One NEW comment per violation — never PATCHed** (the only record that skips the duplicate-prevention search): overwriting would erase the earlier violation's readings, and pooling violations into one comment would let a single stale `:resolved` marker disarm the guard for all later ones. ⚠ **Blocking on re-entry**: Step 0 refuses to resume the stage while any such comment is unresolved — and "resolved" is judged by **position, not presence**: the comment's last non-blank line must be exactly `<!-- guild:auditor-violation:resolved -->`, added *below* the closing marker so the pair stays well-formed. A presence test would be useless here, since the note's own instructions necessarily quote that string. Cleared by adding it as the final line of *that* comment, never by deleting the comment. |
...
Procedure:
1. Run the project's test command (from `config.json` `commands.test` / conventions) as a simple Bash call. **Commands are pre-normalized** (`scan_repo.md` Section 2): a value is either a simple string or an **array** of simple steps. Run each array element as its **own** Bash call in order — never join them with `&&`. The stored form never contains `$(...)`, backticks, `&&`, `||`, `|`, `;`, `&`, newlines, or redirections; if you encounter a raw compound command from an older install, split it yourself and drop any `$(...)` flag before running.
...
⚠ **When the leader cannot tell whether a trigger is met, the two kinds break the tie differently.** A **gate role** (designer · security · infra) exists because the risk it checks is one nobody else on the spine is looking for, and a gate that was never convened leaves **no trace at all** — a diff whose auth surface went unreviewed and a diff that had none read identically afterwards. So an unresolved "does this touch auth / does this have UI surface / is this a deploy change" **convenes the gate role**: the cost is one sub-agent, and the reviewer's own `DONE` is what makes "we looked, there was nothing" a recorded fact instead of an assumption. A **participation role** joins to *produce* work, and convening one that turns out unwarranted spends a full authoring turn on an artifact nobody needs — there the default stays **skip**, but say in one line which role you considered and why the trigger did not fire, so the reader sees a decision rather than an absence. Neither rule licenses guessing at a trigger you could simply check: read the diff surface first (`_execute_spine.md` Step 3.5b matches roles against the actual changed paths), and break the tie only on what is genuinely unresolvable after that.
...
**The execute-stage external auditor is not a roster role.** `_execute_spine.md` Step 3.5a always spawns — **first, alone, and to completion, before any 3.5b gate role** (`_execute_spine.md` Step 3.5a: serialising it is what keeps the working-tree check around it attributable, since 3.5b contains roles that legitimately write files) — a **persona-less** adversarial reviewer (no `.claude/agents/*.md`, no Guild persona — it *does* read `docs/standards/` and the Issue's AC/design, which is what makes its findings anchorable). The 3.5b gate roles that match run only **after** it has finished — never in the same parallel message, since a role that writes files inside the auditor's working-tree check window would make that check unattributable (`_execute_spine.md` Step 3.5). It is deliberately outside this table: it has no trigger to match, no specialty, and no `>>> RESULT <<<` verdict — it returns severity-tagged JSON findings and the leader arbitrates them (Step 4's severity policy). `evolve` HR never adds, retires, or re-tiers it, and `_model_tiering.md` Section C never proposes a default for it.
...
When Guild runs unattended (a supervisor invokes `claude -p "/gld dev <n>"` with `GLD_UNATTENDED=1` set — `batch.md`'s generated script uses `dev`, not `resume`, because `dev` Phase 1 both starts a fresh Issue and resumes a mid-spine one), no human is present to answer a gate. **The leader stands in for the human at in-flow gates — but the human's real authority is deferred to PR review + merge, never removed** (INV1: nothing merges unattended). ⚠ **One exception, in the table below**: the execute-stage external auditor's `BLOCKER` is the single gate verdict the leader may *not* stand in for. This is the plan's sprint principle ("사람 리뷰를 뒤로 미룰 뿐 없애지 않음"). Attended runs are unchanged.
