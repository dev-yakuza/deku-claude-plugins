...
- `.claude/guild/overlay/contribute-candidates.md` — evolve's **"flow friction → upstream-contribution candidate"** flags (friction the spine caused, not locally fixable; `evolve.md`'s Phase 2 flow-friction routing appends one line per flagged item here). Each line has the exact labeled-field shape `- problem: <one-line> — change: <one-line> — evidence: <ref> — evolve #<n>` — parse by these labels, not by positional em-dash splitting (a `problem`/`change` clause may itself contain an em dash). `evolve #<n>` is the **run sequence number** of the evolve run that flagged it (defined once in `evolve.md` **Phase 0 step 3**: prior ledger entries + 1). It is **repo-local** — it names a run of *this* repo's `evolution-log.md` and means nothing upstream — so use it **locally only** (the dedup search below, and step 4's line match) and **strip it before sending** (step 2). It is a backpointer, not a unique key: a proposal flagged during a `--dry-run` carries a number that run never recorded, so two candidate lines can share one. Absent/empty → no flags this cycle, not an error.
