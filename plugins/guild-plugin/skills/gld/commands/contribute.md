...
1. **Deterministic secret scan first** — do not rely on judgment alone for the one class of leak that's mechanically detectable. Run the candidate text through the same high-signal patterns the commit gate uses (`<<SKILL_DIR>>/gates/gate_precommit.py` `INLINE_SECRET_RES` — AWS keys, PEM private-key headers, Google API keys, Slack/GitHub tokens, Stripe live keys): grep the candidate for those patterns (its own Bash call) before anything is drafted for sending. Any hit → treat the candidate as blocked until the human confirms the match was removed; never pass a hit through to sub-step (2) below "on the assumption it'll get caught there."
