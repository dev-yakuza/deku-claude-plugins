...
5. **예측 먼저 (D 예측-후-공개 · optional)**: for a stop with a non-obvious answer, **first ask the human to reason** — *"이 구조라면 X는 어디서 처리될까요?"* / *"여기서 뭐가 위험해 보이세요?"* — then reveal. The predict→compare gap is where the model sticks — the same surprise-ranking principle the growth loop uses, applied to the human. Skip for purely descriptive stops.
