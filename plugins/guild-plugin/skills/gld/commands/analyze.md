...
- Contents: interpretation chosen (discuss), **readiness line** (`_readiness.md` Section D — level tokens ASCII, surrounding prose in `config.language`; e.g. `Readiness: Goal=clear · Constraint=clear · Success=partial (가정: ...)`), feature list, **acceptance criteria**, work-type classification, priorities. Append a `<details>` pre-flight trace (`_preflight.md` Section C).
