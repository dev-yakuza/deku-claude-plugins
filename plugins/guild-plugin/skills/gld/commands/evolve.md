...
- **D. Variation–selection–inheritance** — **selection** is embodied (Phase 4 adversarial panel + HR replace/retire pick winners on ground truth); full **population variation** (competing agent-def variants judged in parallel) stays **deferred** (항목 5 — bio-mimicry guard: adopt only when it beats the single-attempt baseline, which it can't demonstrate below scale). **Concrete trigger to build it**: (a) contribute/360 exist ✅ *and* (b) a repo/fleet large enough that N-variant judging amortizes — heuristic: ≥N candidate habits per run *and* a token budget where N× variant-generation + judging is affordable (tie to `budget`/scale, not a flag). Until (b), the single-attempt path is the correct baseline; building an OFF-by-default variant engine now would be unvalidated dead code.
