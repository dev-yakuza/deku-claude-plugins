...
**`edit` verdict bar (keep it meaningful, not the default)**: tell every spawned lens explicitly — `edit` is for a **substantive** problem with the proposed wording/scope (it materially changes what the rule/fact/habit will do, or fixes something that would otherwise mislead). A lens that would merely polish phrasing, tighten wording, or make a cosmetic style tweak must return `keep` and put the nit in `reason` as an aside (not the `edit` field) — it does not change what gets applied. **Rationale**: lenses are instructed to hunt for problems (Phase 4 opening), so left unconstrained they tend to always find *some* wording nit, which would make `edit` — and the `(패널 수정본 기준)` qualifier it drives on the `패널 판정: 적용` label — the default for nearly every item and drain it of signal. Reserve it for proposals that genuinely changed shape under review.
...
1. Present **only that one item**, in **plain language** (per the rule at the top of this file — no "Tier", "ladder rung", "INV", "Phase", or bare `evidence:`/`lens:` fields). Lead with an explicit, standardized **verdict label** derived from the lens verdicts — do not bury it in prose:
   **The label is binary — always exactly `패널 판정: 적용` or `패널 판정: 거절`, never a third state.** Hedges ("신중히 검토", "검토 필요", "판단 요망", "주의") are **banned** — they hand the call back to the human, which is what this label exists to spare them. A qualifier in parentheses may add context, but it never softens the verb:
   - all lenses `keep` (with no `edit` field attached — see below) → **`패널 판정: 적용`**
   - any lens carries an edit/refinement (and none dissenting) → **`패널 판정: 적용 (패널 수정본 기준)`** — the verdict stays 적용; the qualifier only says *which version* is on the table. Fold the suggested edit into the 왜/무엇을 lines (and the diff). This check has two shapes depending on the item: **single-draft items** (Phase 4's default 3-lens/1-lens review) — a lens's top-level `verdict` is literally `"edit"`. **Population-mode bundles** (Phase 3 generated 3 candidates) — a lens's `verdict` is only ever `keep`/`drop` (never `"edit"` — see the bundle shape above); the refinement instead lives in that lens's optional `edit` **field** alongside a `keep` verdict. Treat both the same way: `verdict == "edit"` OR (`verdict == "keep"` AND an `edit` field is present) → this label. Missing this second form would mean a population-mode refinement silently falls into the "all keep" bucket above and never reaches the human.
   - a lens `drop`ped but was outvoted (survived only because it wasn't a majority) → **`패널 판정: 거절 (<반대 렌즈> 반대)`** — a real dissent on a self-modification is enough to recommend **against** it, even though the majority kept it. ⚠ This is deliberately **conservative and deliberately decisive**: saying 적용 here would make the label constant (every item that reaches Phase 5 survived the panel) and therefore worthless, while a hedge would just relay the panel's split to the human as homework. The 우려 line below carries the dissenting reason so the human can **override to 적용** on the spot — that override is expected and normal, not a fight with the tool.
   ...
   - **우려** (required whenever the label is `패널 판정: 거절`): one plain sentence naming the dissenting lens's concern, so the human's accept/reject/edit choice is informed (e.g. "우려: 기존 규칙과 살짝 겹칠 수 있다는 의견이 있었어요").
...
2. Ask the human to choose **accept / reject / edit** for *this item alone* and **wait for the reply** before showing the next item — never list several items in the same message and ask for a combined decision, and never move to item N+1 until item N is resolved. **Repeat the verdict label in the question itself** (the body above may be long enough that the human has forgotten it by the time they reach the decision), using the **same words as the options** so the recommendation points at one of them — e.g. *"패널 판정은 **적용**입니다 (패널 수정본 기준). 적용(accept) / 거절(reject) / 수정(edit) 중 무엇으로 할까요?"* or *"패널 판정은 **거절**입니다 (안전성 렌즈 반대). 적용(accept) / 거절(reject) / 수정(edit) 중 무엇으로 할까요?"*.
