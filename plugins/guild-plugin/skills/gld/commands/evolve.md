...
**Rule scorecard (항목 3b — symmetric with the per-agent scorecard; feeds rule HR)**: score each **stack-specific / boundary** rule from durable data (no new instrumentation): read `.claude/guild/memory/gate-firings.jsonl` (the gate's firing log — count firings, block/warn split), cross-reference `.claude/guild/gates/dismissed.md` (accepted-risk = the human overrode → a **noise/false-positive** proxy), and `scan_git` for the **guarded-area churn** (is the area it protects even changing?). Per rule: **firing count · dismissal rate · estimated real-hits** (fired → human *fixed* the code) **vs false-positives** (fired → *dismissed*) **· area churn**. Trend across runs (ledger). Same weak-sample caveat + Axis-2 trend gate as the agent scorecard. ⚠ **Do NOT score the secret / verification gates for retirement** — they are exempt (INV2, see Phase 2 rule HR); the scorecard covers only demotable stack-specific/boundary rules.
