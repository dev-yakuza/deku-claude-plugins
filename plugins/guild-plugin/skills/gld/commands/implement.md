...
- Tech Lead `BLOCKED` (non-conformance), a **gate `BLOCKED`** (e.g. security vulnerability), OR evidence contradicts green → before looping back, apply the **stagnation guard** (`_stagnation.md`): compare this reason against the immediately-prior loop-back's reason for this Issue (if any). **Same root cause repeated** → stagnation — escalate immediately (`_stagnation.md` Section B) instead of consuming another attempt. **Different concern** → **defined loop back to execute**: re-invoke the developer (Step 1) with the specific concern. Bounded — after ~2 loops without resolution, return `NEEDS_HUMAN: <one-line>`.
