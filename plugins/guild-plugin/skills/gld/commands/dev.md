...
- **Human approval is always the final gate.** The human approving the PR is the external reviewer of record (INV1) — Guild collaboration (tech-lead conformance, tester-first, and execute Step 3.5a's always-on adversarial audit of the diff) provides internal review throughout, and `/gld review`'s Step 2.5 adds an independent agent-based adversarial pre-scan as a further, advisory 2nd opinion before that human approval (never a replacement for it).
