...
" <count> <runs>
```
(After `/gld evolve`, `runs` advances so the *next review of any PR* re-arms once via (b) — not just the next review of the same PR. Consolidation *usually* drops `count`, but **resolved-elsewhere residuals** can keep it pinned at 충분 — the `+5` threshold in (c) prevents that floor from re-firing on every subsequent correction, and evolve's Phase 7 residual-hygiene archives those residuals so the floor itself shrinks over time.)
