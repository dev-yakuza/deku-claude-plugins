...
**Cooldown — nudge once per evolve cycle (re-fire only on growth).** The 충분 state persists across many reviews, so an unconditional nudge fires *every* review until the human runs evolve — naggy. Gate it on a tiny local state file `.claude/guild/memory/review-nudge-state.json` (gitignored working tier — same tier as `ground-truth.jsonl`), **keyed by PR number**: `{"<pr>": {"count": <deduped count at last nudge for this PR>, "runs": <ledger run count at last nudge>}}`. ⚠ **Keying by PR matters**: two `/gld review` runs on different PRs can run close together in the same clone; a flat single `{count, runs}` with no PR key (an earlier version of this doc used) lets one PR's read-then-write race with the other's and silently clobber or wrongly suppress its nudge. **This narrows the race, it doesn't eliminate it** — the read-modify-write below still has no file lock, so two `/gld review` runs finishing at nearly the same moment can still lose one of their two updates (both read the file before either writes, so the second write overwrites the first's change with its own stale copy of everything else). Accepted as-is: the consequence is only a missed or repeated *advisory* nudge for one PR, self-heals on the next review of that PR, and adding real file locking here would be disproportionate to what's at stake. Read the whole file as its own Bash call (absent file, or no entry for this PR → no prior nudge for this PR):
```bash
cat .claude/guild/memory/review-nudge-state.json
```
Look up `state["<this PR number>"]` in what you just read (missing → treat as `{count: 0, runs: 0}`, i.e. no prior nudge). **Nudge iff 충분 AND** any of: **(a)** no prior state for this PR (first time), **(b)** `runs > state.runs` (an evolve ran since this PR's last nudge → new cycle, re-arm), **(c)** `count >= state.count + 5` (a full 충분-worth of *net-new* signal piled up since this PR's last nudge). Otherwise **stay silent** (same cycle, no new material for this PR — already nudged). ⚠ **(c) uses `+5`, not a bare `count > state.count`**: when **resolved-elsewhere residuals** (signals closed by a human edit / test fix / refutation rather than an evolve apply) keep the count pinned at 충분, a bare `>` re-fires the nudge on the *very first* new correction after every evolve — the naggy case the human reported. Requiring a full 충분-worth of growth restores the intended "re-fire only on real accumulation." On firing, read-modify-write **only this PR's key**, preserving every other PR's entry (its own Bash call, best-effort — never blocks the recap):
```bash
python3 -c "
import json, sys
...
state[sys.argv[1]] = {'count': int(sys.argv[2]), 'runs': int(sys.argv[3])}
