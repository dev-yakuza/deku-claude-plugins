...
- **External auditor (always — fresh eyes, no Guild persona → unbiased, 외부자)**: `subagent_type: general-purpose`, `model: sonnet`, `description: adversarial review #$1`, prompt:
  ...
  > that human as a plain-language **binary** verdict keyed straight off the level
  > you choose: BLOCKER and MAJOR are presented as "fix it in this PR", MINOR as
  > "no fix needed — ship as is". There is no in-between label, so grade by
  > consequence knowing a MINOR is what tells the human not to act.
  >
...
5. **확인할 점 (D 예측-후-공개 옵션)**: for a risky unit, first **invite the human to predict** — *"이 유닛에서 뭐가 위험해 보이세요?"* — then reveal the **Step 2.5 adversarial findings that map to this unit** (by file:line, highest severity first, axis-tagged `[standards]`/`[spec]`). **Present each finding pre-translated, never raw JSON/jargon** — one plain-Korean line explaining what's wrong and why it matters, followed by an explicit **binary verdict** derived straight from severity:
   ```
   [standards] lib/x.dart:42 — <평이한 설명: 무엇이 왜 문제인지, 방치하면 어떻게 되는지>
   → 수정 필요 (BLOCKER) — <한 줄 근거: 왜 이번 PR에서 고쳐야 하는지>
   ```
   **The verdict is binary — exactly one of `수정 필요` / `수정 불필요`, never a hedge.** Hedging words ("선택", "참고용", "권장", "가능하면", "검토 바람") are **banned in this line** — they hand the call back to the human, which is exactly what this line exists to prevent. The map is mechanical, not a fresh judgment:
   ...
   - `MINOR` → **수정 불필요** — 이대로 머지해도 됩니다(MINOR는 정의상 동작·노출·데이터·가용성·AC 어느 쪽에도 결과가 없는 건). 한 줄로 **왜 결과가 없는지** 근거를 붙여 인간이 동의하지 않으면 뒤집을 수 있게 하고, "고쳐달라고 하시면 바로 적용합니다" 한 마디를 덧붙일 것.
   Every presented finding carries exactly one verdict — never present a finding without one. A leader scrutiny note goes through the **same** severity core (`<!-- guild:severity-core -->`): grade it BLOCKER/MAJOR/MINOR first, then map — an ungraded "한번 보세요" is not a verdict and must not be presented as one. The wording follows `config.language`; the binary itself (fix / don't fix) holds in every language. **The human should never have to ask "쉽게 설명해 주세요" or "수정이 필요한가요?"** — that translation + recommendation is Claude's job on every reveal, not on request. The predict→compare gap is where learning happens. Nothing found + no concern → say so and move on.
