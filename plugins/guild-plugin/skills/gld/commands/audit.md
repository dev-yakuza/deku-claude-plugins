...
**Data-sufficiency banner** (`<<SKILL_DIR>>/commands/atoms/_data_sufficiency.md`): compute the **cheap proxy** (Section B — `ground-truth.jsonl` deduped count + ledger run count; one read each — the same inputs review uses, since dimensions E/F aren't gathered until Phase 1 and the budget rule forbids new scans just for the banner). Print the banner (Section D) at the **top of the report**; when the full report renders, **refine** the tier with the reverts/CI-gaps that Phase 1 dimensions E/F surface (fold them into the count then). ⚠ **audit never blocks on it** — a low tier is surfaced as a finding (*"evolve는 아직 이릅니다 — 신호 N개·run M회"*) but the full diagnosis continues; audit is precisely how the human learns they are thin and what to do next.
