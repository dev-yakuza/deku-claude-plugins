...
Post/print the audit report (grades + findings by dimension, most-severe first). **When `$1` scoped this run to one dimension** (Phase 1), only route the nudge relevant to that dimension — a `codebase`-scoped run never ran dimensions A–E, so it has no basis to suggest `/gld evolve`, and a `harness`/`team`/`knowledge`/`standards`/`evolution`-scoped run never ran dimension F, so it has no basis to recommend a refactor Issue. Then route — **nudges, never auto-run** (plan §9):
